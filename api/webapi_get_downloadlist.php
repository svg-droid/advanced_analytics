...
$num_rec_per_page=10;
$array_temp['code']=1;
$array_temp['message']="Download List";
$array_final=array();
...
$row_count_data = numRows("select fk_user_id,fk_chapter_id,id,status from tbl_download where fk_user_id='".$userid."' and status=1");

$total_records = $row_count_data; 
$total_pages = ceil($total_records / $num_rec_per_page); 

$main_qry = ("select fk_user_id,fk_chapter_id,id,status from tbl_download where fk_user_id='".$userid."' and status=1 LIMIT ".$start_from.",".$num_rec_per_page);
...
if($row_count)
{
	...
	foreach($str as $res1)
	{
		$row = fetchRow("SELECT fk_class_id,id,status,fk_subject_id,chapter_name,pdf_file FROM tbl_chapter WHERE id=".$res1['fk_chapter_id']);
		...
		$array_temp1['content'] = $row['chapter_name'];
