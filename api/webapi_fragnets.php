...
if($checkuser>0)
{
	...
	/*if($token>0)
    {*/
	     $row=fetchRows("SELECT * FROM tbl_fragnets left join tbl_fragnets_image on tbl_fragnets_image.fk_fragnetid = tbl_fragnets.fragnetid WHERE statuscheck = 1");
				if($row)
				{
				$array_final=array();
				$array_final1=array();
				...
				{
					$array_temp1['title']=strip_tags(urldecode($fragnets['fragnettitle']));
					$array_temp1['mainimage']=COFIG_SITE_URL_IMG."fragnets_img/".$fragnets['image'];
					$array_temp1['thumbimage']=COFIG_SITE_URL_IMG."fragnets_img/thumb/".$fragnets['thumbimage'];
