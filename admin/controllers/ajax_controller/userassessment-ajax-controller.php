...
<?php
  ...
  //  exit;
		$TotalCountqry = $modelObj->numRows("select * FROM tbl_userresults where fk_userid ='$getuserid'");
...
//exit;
    if($sSearch!=''){
			...
			$Filteredqry = $modelObj->numRows("SELECT tbl_users.name,tbl_assessments_category.categoryname,tbl_userresults.* FROM tbl_userresults left join tbl_assessments_category on tbl_assessments_category.categoryid = tbl_userresults.fk_category_id left join  tbl_users on tbl_users.id= tbl_userresults.fk_userid  where id ='$getuserid' and statuscheck !=2 and (categoryname LIKE '%".$sSearch."%' || totalnoquestions LIKE '%".$sSearch."%' || totalrightanswer LIKE '%".$sSearch."%')");
			...
			$list_activity_log = $modelObj->fetchRows("SELECT tbl_users.name,tbl_assessments_category.categoryname,tbl_userresults.* FROM tbl_userresults left join tbl_assessments_category on tbl_assessments_category.categoryid = tbl_userresults.fk_category_id left join  tbl_users on tbl_users.id= tbl_userresults.fk_userid  where id ='$getuserid' and statuscheck !=2 and (categoryname LIKE '%".$sSearch."%' || totalnoquestions LIKE '%".$sSearch."%' || totalrightanswer LIKE '%".$sSearch."%') ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
			} else {

			$Filteredqry = $modelObj->numRows("SELECT tbl_users.name,tbl_assessments_category.categoryname,tbl_userresults.* FROM tbl_userresults left join tbl_assessments_category on tbl_assessments_category.categoryid = tbl_userresults.fk_category_id left join  tbl_users on tbl_users on tbl_users.id= tbl_userresults.fk_userid where id = '$getuserid' and statuscheck !=2");

			$list_activity_log = $modelObj->fetchRows("SELECT tbl_users.name,tbl_assessments_category.categoryname,tbl_userresults.* FROM tbl_userresults left join tbl_assessments_category on tbl_assessments_category.categoryid = tbl_userresults.fk_category_id left join  tbl_users on  tbl_users.id= tbl_userresults.fk_userid where id ='$getuserid' and statuscheck !=2 ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
		}
		...
		foreach($list_activity_log as $result){
			...
			$query="SELECT tbl_users.name,tbl_assessments_category.categoryname,tbl_userresults.* FROM tbl_userresults left join tbl_assessments_category on tbl_assessments_category.categoryid = tbl_userresults.fk_category_id left join  tbl_users on tbl_users.id= tbl_userresults.fk_userid where id ='$getuserid' and statuscheck !=2";
			$results=$modelObj->fetchRows($query);

			$cms_id = $result['userresultid'];
			$title = urldecode($result['categoryname']);
      $email = $result['useremail'];
      $totalquestions = $result['totalnoquestions'];
      $totalrightanswer = $result['totalrightanswer'];
...
/*	  $image = $result['image'];
      ...
      $cms4='<td>'.$totalrightanswer.'</td>';
		...
		//	$cmsemail='<td><img height="100" width="100" src='.$imagesrc.'></td>';
			...
			<a class="table-actions" href="index.php?pid=usertestresult&user='.$cms_id.'"  title="Assessment Results">
		...
		$option='<td class="actions">
			...
			$Arr['aaData'][] = array($checkbox,$cms2,$cms3,$cms4,$option);
		}
	...
	<div class="col-lg-12">
      ...
      <?php
			...
			?>
				...
				<!--		<li>
                            <a href="javascript:void(0)" onclick="statusactive('Active','active','userassessment')"><i class="icon-ok-sign"></i>Active</a>
						</li>
                        <li>
                            <a href="javascript:void(0)" onclick="statusinactive('Inactive','inactive','userassessment')"><i class="icon-ok-circle"></i>Inactive</a>
						</li> -->
						 <?php if($moduleresult['p_field4'] == 1){ ?>
						<li>
							<a href="javascript:void(0)" onclick="deleteselected('Delete','delete','userassessment')"><i class="icon-remove"></i>Delete</a>
						</li>
						<?php } ?>
					</ul>
				</div>
            ...
            <th lass="hidden-xs" style="color: #007aff;">Total Questions</th>
            <th lass="hidden-xs" style="color: #007aff;">Total  Right Answer</th>
	...
	<script>
		$(document).ready(function() {
			$('#tbl_data_display').dataTable({
				...
				"sAjaxSource": site_url + 'controllers/ajax_controller/userassessment-ajax-controller.php?action=displaydata&userviewid=<?php echo $_SESSION['getuserid']; ?>'
			});
		});
	</script>
