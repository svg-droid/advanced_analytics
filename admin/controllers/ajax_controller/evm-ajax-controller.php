<?php
  ...
  $upload_dir2 =$_SESSION['SITE_IMG_PATH']."evm_img/thumb/";
  ...
  // $upload_dir3 =$_SESSION['SITE_IMG_PATH']."fragnets_img/";
   ...
   $qry="SELECT * FROM tbl_evm left join tbl_evm_image on tbl_evm_image.fk_evmid = tbl_evm.evmid where  tbl_evm.evmid=".$id;
    $result=$modelObj->fetchRow($qry);

?>
<div class="col-md-12">
    <div class="widget-content padded">
		<dl><b>Title : </b> <?php echo urldecode($result['evmttitle']); ?></dl>
    <?php
  if($result['image'] == '')
  {
  ...
  $imageviewthumb =  COFIG_SITE_URL_IMG."evm_img/thumb/".$result['thumbimage'];
  }
     ?>
...
$imageview =  COFIG_SITE_URL_IMG."evm_img/".$result['image'];
}
      ?>
...
<?php
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == "displaydata"){
		...
		$TotalCountqry = $modelObj->numRows("select * FROM tbl_evm");
    ...
    else  if(strtolower($sSearch) == 'inactive')
      {
        ...
        $sSearchactive = $sSearch;
				...
				$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_evm  where statuscheck !=2 and (evmttitle LIKE '%".$sSearch."%' || statuscheck LIKE '%$sSearchactive%')");
			...
			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_evm  where statuscheck !=2 and (evmttitle LIKE '%".$sSearch."%' || statuscheck LIKE '%$sSearchactive%') ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
			} else {
			$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_evm left join tbl_evm_image on tbl_evm_image.fk_evmid = tbl_evm.evmid where statuscheck !=2");

			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_evm left join tbl_evm_image on tbl_evm_image.fk_evmid = tbl_evm.evmid  where statuscheck !=2 ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
		}
		...
		foreach($list_activity_log as $result){
			...
			$query="SELECT  FROM tbl_evm left join tbl_evm_image on tbl_evm_image.fk_evmid = tbl_evm.evmid";
			$results=$modelObj->fetchRows($query);

			$cms_id = $result['evmid'];
			$title = urldecode($result['evmttitle']);
...
/*	  $image = $result['image'];
      ...
      $imagesrc ='http://www.placehold.it/200x150/EFEFEF/AAAAAA&text=no+image';
		...
		//	$cmsemail='<td><img height="100" width="100" src='.$imagesrc.'></td>';
			...
			$option .='<a class="table-actions" href="javascript:void(0)" onclick="edit('.$cms_id.',\'evm\')" title="edit">
				<i class="icon-edit"></i>
			</a>
		...
		$option='<td class="actions">
			...
			<a class="table-actions" href="javascript:void(0)" onclick="deleteuser('.$cms_id.', \'delete\', \'evm\')" title="Delete">
...
<?php
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val){
			$qry="UPDATE tbl_evm SET  statuscheck=2 WHERE evmid='".($val)."'";
...
<?php
	if (isset($_POST['viewdiv']) && $_POST['viewdiv'] != '') {
		...
		$qry = "SELECT * FROM tbl_evm left join tbl_evm_image on tbl_evm_image.fk_evmid = tbl_evm.evmid where statuscheck !=2";
			...
			?>
				...
				<div class="btn-group hidden-xs pull-right ajesh_margin_right_10">
					...
					<ul class="dropdown-menu">
						<li>
                            <a href="javascript:void(0)" onclick="statusactive('Active','active','evm?')"><i class="icon-ok-sign"></i>Active</a>
						</li>
                        <li>
                            <a href="javascript:void(0)" onclick="statusinactive('Inactive','inactive','evm?')"><i class="icon-ok-circle"></i>Inactive</a>
						</li>
						 <?php if($moduleresult['p_field4'] == 1){ ?>
						<li>
							<!-- <a href="javascript:void(0)" onclick="deleteselected('Delete','delete','evm?')"><i class="icon-remove"></i>Delete</a> -->
						</li>
						<?php } ?>
					</ul>
				</div>
	...
	<script>
		$(document).ready(function() {
			$('#tbl_data_display').dataTable({
				...
				"sAjaxSource": site_url + 'controllers/ajax_controller/evm-ajax-controller.php?action=displaydata'
			});
		});
	</script>
...
<?php
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
		...
		$qry="UPDATE tbl_evm SET statuscheck=2 WHERE 	evmid='".$_POST['id']."'";
			...
			<div class="widget-content padded">
				<form name="form_testadd" id="form_testadd" method="post" enctype="multipart/form-data" action="" class="form-horizontal">
					...
					<div class="form-group">
						...
						<div class="col-md-7">
						<input class="form-control" type="text" name="evmtitle"  id="evmtitle" value="<?php echo htmlentities(urldecode($data['evmttitle'])); ?>" />
                    ...
                    <?php if($data['image'] == '' ){ ?>
                      ...
                      <?php } else {
                        if($controller_class->isAws()){
                          $url = COFIG_SITE_URL_IMG.'evm_img/thumb/'.$data['thumbimage'];
                          }else{
