...
<div class="col-md-12">
    <div class="widget-content padded">
		...
		<dl><b>Option 4 : </b> <?php echo  urldecode($result['option4']); ?></dl>
		 <dl><b>Right Answer : </b> <?php echo  "option ".$result['rightanswer'] ; ?></dl>
     <?php
if($result['image'] == '')
{
...
<?php
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == "displaydata"){
		...
		$TotalCountqry = $modelObj->numRows("select * FROM tbl_assessments_tests");



		if($sSearch!=''){
				$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_assessments_tests  where (questions LIKE '%".$sSearch."%')");
			...
			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_assessments_tests inner join tbl_assessments_category on tbl_assessments_category.categoryid = tbl_assessments_tests.fk_category_id  where (questions LIKE '%".$sSearch."%' || categoryname LIKE '%".$sSearch."%') ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
			} else {
			$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_assessments_tests inner join tbl_assessments_category on tbl_assessments_category.categoryid = tbl_assessments_tests.fk_category_id where statuscheck !=2");

			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_assessments_tests inner join tbl_assessments_category on tbl_assessments_category.categoryid = tbl_assessments_tests.fk_category_id where statuscheck !=2 LIMIT ".$iDisplayStart.",".$iDisplayLength."");
		}
		...
		foreach($list_activity_log as $result){
			...
			$option .='<a class="table-actions" href="javascript:void(0)" onclick="edit('.$cms_id.',\'test\')" title="edit">
			<span>  edit</span>
			</a>';


			$option .='<a class="table-actions" href="javascript:void(0)" onclick="deleteuser('.$cms_id.', \'delete\', \'cms?\')" title="Delete">
		...
		$option='<td class="actions">
			...
			$Arr['aaData'][] = array($checkbox,$cms,$cmsemail,$status_new,$option);
		}
			...
			?>
				...
				<div class="btn-group hidden-xs pull-right ajesh_margin_right_10">
					...
					<ul class="dropdown-menu">
						<li>
                            <a href="javascript:void(0)" onclick="statusactive('Active','active','test')"><i class="icon-ok-sign"></i>Active</a>
						</li>
                        <li>
                            <a href="javascript:void(0)" onclick="statusinactive('Inactive','inactive','test')"><i class="icon-ok-circle"></i>Inactive</a>
						</li>
						 <?php if($moduleresult['p_field4'] == 1){ ?>
						<li>
							<a href="javascript:void(0)" onclick="deleteselected('Delete','delete','test')"><i class="icon-remove"></i>Delete</a>
						</li>
						<?php } ?>
					</ul>
				</div>
	...
	<script>
		$(document).ready(function() {
			$('#tbl_data_display').dataTable({
				...
				"sAjaxSource": site_url + 'controllers/ajax_controller/test-ajax-controller.php?action=displaydata'
			});
		});
	</script>
			...
			<div class="widget-content padded">
				<form name="form_testadd" id="form_testadd" method="post" enctype="multipart/form-data" action="" class="form-horizontal">
					...
					<div class="form-group">
						...
						<div class="col-md-7">
						<input class="form-control" type="text" name="questions" maxlength="40" id="questions" value="<?php echo urldecode($data['questions']); ?>" />
					...
					<div class="form-group">
						...
						<div class="col-md-7">
						<input class="form-control" type="text" name="option1" maxlength="40" id="option1" value="<?php echo urldecode($data['option1']); ?>" />
					...
					<div class="form-group">
						...
						<div class="col-md-7">
						<input class="form-control" type="text" name="option2" maxlength="40" id="option2" value="<?php echo urldecode($data['option2']); ?>" />
					...
					<div class="form-group">
						...
						<div class="col-md-7">
						<input class="form-control" type="text" name="option3" maxlength="40" id="option3" value="<?php echo urldecode($data['option3']); ?>" />
					...
					<div class="form-group">
						...
						<div class="col-md-7">
						<input class="form-control" type="text" name="option4" maxlength="40" id="option4" value="<?php echo urldecode($data['option4']); ?>" />
