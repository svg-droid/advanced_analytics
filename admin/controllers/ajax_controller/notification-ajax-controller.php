<?php 
	...
	include('../../models/notification-model.php');
    include('../../includes/thumb_new.php');
    include('../../includes/resize-class.php');
    include('../common-controller.php');
	include('../notification-controller.php');
    include('../../models/ajax-model.php');
    $modelObj = new AjaxModel();
	$controller_class = new NotificationController();
	$upload_dir1 =$_SESSION['SITE_IMG_PATH']."notification/";
    $upload_dir =$_SESSION['SITE_IMG_PATH']."notification/";
    $upload_dirthumb =$_SESSION['SITE_IMG_PATH']."notification/thumb/";
	

if(isset($_POST['view']) && $_POST['view'] != ''):
    $id = $_POST['id'];
    $qry="SELECT * FROM tbl_notification where status!=2 and id='".$id."'";
...
<div class="col-md-12">
    <div class="widget-content padded">
		...
		<dl><b>Notification Status: </b>  <?php echo $result['notification_status']; ?></dl> 
	</div>
  </div>
<?php endif; ?>
<?php
...
if(isset($_REQUEST['action']) && $_REQUEST['action'] == "displaydata"){
  ...
  $TotalCountqry = $modelObj->numRows("select * FROM tbl_notification WHERE status!=2");
  ...
  $user = $modelObj->fetchRows("SELECT id FROM tbl_chapter WHERE status!=2 and chapter_name LIKE '%".$sSearch."%'");
	...
	if($sSearch!=''){
		$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_notification WHERE status!=2 AND (cr_date LIKE '%".$sSearch."%' $query1 $query2 $query3 $query4)");
		
		$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_notification WHERE status!=2 AND (cr_date LIKE '%".$sSearch."%' $query1 $query2 $query3 $query4) ORDER BY cr_date ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");	
	} else {
		$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_notification WHERE status!=2");
		
		$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_notification WHERE status!=2 ORDER BY cr_date ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");	
	}
  ...
  foreach($list_activity_log as $result){
   ...
   $cms_status = $result['status'];
		...
		$option='<td class="actions">
					...
					<a class="table-actions" href="javascript:void(0)" onclick="deleteuser('.$cms_id.', \'delete\', \'notification?\')" title="Delete">
							   <i class="icon-trash"></i>
					</a>
					</div>
				</td>';
   
   $Arr['aaData'][] = array($checkbox,$class_na,$subject_na,$chapter_na,$user_name_na,$cr_date_na,$status,$option);
  } 
...
<?php
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_notification SET status=1 WHERE id=".($val);
...
<?php
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_notification SET status=0 WHERE id=".($val);
			$result =$modelObj->runQuery($qry);
		}
	}
?>
<?php
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val){
			$qry="UPDATE tbl_notification SET status=2 WHERE id='".$val."'";
			$result=$modelObj->runQuery($qry);
		}
	}
?>
...
<?php
	if(isset($_POST['hid_add']) && $_POST['hid_add'] != ''){		
		$checkexists="SELECT id FROM tbl_notification where status!=2 and chapter_name='".($_POST['txt_chapter_name'])."'";
...
<?php
    ...
    $qry = "SELECT * FROM tbl_notification where status!=2 order by cr_date desc";
...
<div class="col-lg-12">
    <div class="widget-container fluid-height clearfix">
        <div class="heading"> <i style="cursor:default;" class="icon-table"></i> Notification List
            <?php if($result !='' ): ?>
            <div class="btn-group hidden-xs pull-right ajesh_margin_right_10">
                ...
                <ul class="dropdown-menu">
                    <li>
                            <a href="javascript:void(0)" onclick="statusactive('Active','active','notification?')"><i class="icon-ok-sign"></i>Active</a>
                        </li>
                        <li>
                            <a href="javascript:void(0)" onclick="statusinactive('Inactive','inactive','notification?')"><i class="icon-ok-circle"></i>Inactive</a>
                        </li>
						<li>
						<a href="javascript:void(0)" onclick="deleteselected('Delete','delete','notification?')"><i class="icon-remove"></i>Delete</a>
					  </li>
                </ul>
            </div>
            <?php endif;?> </div>
        <div class="widget-content padded clearfix">
            <table class="table table-bordered table-striped" id="tbl_data_display">
                <thead>
                    ...
                    <th class="hidden-xs" style="color: #007aff;"> Notification Date </th>
...
<script>
    $(document).ready(function() {
        $('#tbl_data_display').dataTable({
            ...
            "sAjaxSource": site_url + 'controllers/ajax_controller/notification-ajax-controller.php?action=displaydata'
        });
    });
</script>
...
<?php
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
		...
		$qry="UPDATE tbl_notification SET status=2 WHERE id='".$_POST['id']."'";
		$result=$modelObj->runQuery($qry);
...
if(isset($_POST['edit']) && $_POST['edit'] != ''){
	$qry="SELECT * FROM tbl_notification WHERE id=".$_POST['id'];
