<?php
    ...
    include('../../models/common-model.php');
	  include('../../models/risk-model.php');
    include('../../includes/thumb_new.php');
    include('../../includes/resize-class.php');
    include('../common-controller.php');
	  include('../risk-controller.php');
    include('../../models/ajax-model.php');
    $modelObj = new AjaxModel();
	$controller_class = new CommonController();
	$upload_dir1 =$_SESSION['SITE_IMG_PATH']."risk/";
  $upload_dir =$_SESSION['SITE_IMG_PATH']."risk_img/";
  $upload_dir2 =$_SESSION['SITE_IMG_PATH']."risk_img/thumb/";
  $upload_dirthumb =$_SESSION['SITE_IMG_PATH']."risk/thumb/";
  // $upload_dir3 =$_SESSION['SITE_IMG_PATH']."fragnets_img/";

	if(isset($_POST['view']) && $_POST['view'] != ''):
    $id = $_POST['id'];

   $qry="SELECT * FROM tbl_risk where  riskid=".$id;
    $result=$modelObj->fetchRow($qry);

?>
<div class="col-md-12">
    <div class="widget-content padded">
		<dl><b>Title : </b> <?php echo urldecode($result['risktitle']); ?></dl>
    <?php
		...
		$imageviewthumb =  COFIG_SITE_URL_IMG."risk_img/thumb/".$result['thumbimage'];
		}
		     ?>
		...
		$imageview =  COFIG_SITE_URL_IMG."risk_img/".$result['image'];
		}
      ?>
...
<?php
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == "displaydata"){
		...
		$TotalCountqry = $modelObj->numRows("select * FROM tbl_risk");
    ...
    else  if(strtolower($sSearch) == 'inactive')
      {
        ...
        $sSearchactive = $sSearch;
		  ...
		  $Filteredqry = $modelObj->numRows("SELECT * FROM tbl_risk  where statuscheck !=2 and (risktitle LIKE '%".$sSearch."%' || statuscheck LIKE '%$sSearchactive%')");
			...
			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_risk  where statuscheck !=2 and (risktitle LIKE '%".$sSearch."%' || statuscheck LIKE '%$sSearchactive%') ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
			} else {
			$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_risk where statuscheck !=2");

			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_risk where statuscheck !=2 ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
		}
		...
		foreach($list_activity_log as $result){
			...
			$cms_id = $result['riskid'];
			$title = urldecode($result['risktitle']);
		  	$image = $result['image'];
		...
		//	$cmsemail='<td><img height="100" width="100" src='.COFIG_SITE_URL_IMG.'fragnets_img/'.$image.'></td>';
			...
			$option .='<a class="table-actions" href="javascript:void(0)" onclick="edit('.$cms_id.',\'risk\')" title="edit">
				<i class="icon-edit"></i>
			</a>';
    ...
    if(isset($_FILES["txt_addphoto"]["tmp_name"])){
        ...
        if(isset($_FILES["txt_addphoto2"]["tmp_name"])){
      				...
      				if($_FILES["txt_addphoto2"]["tmp_name"]!=''){
      					...
      					$insertimagethumb 	= 'riskthumb'.time().".jpg";
			...
			$post['risktitle']=urlencode($_POST['fragnetstitle']);
		  	$post['statuscheck']=1;
      		$post['image'] = $insertimage;
      		$post['thumbimage'] = $insertimagethumb;

	   	   	$addfaq=$controller_class->addData('tbl_risk',$post);
...
<?php
	if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){
   	...
   	if(isset($_FILES["txt_addphoto"]["tmp_name"])){
	    ...
	    if($insertimagethumb == '')
	    {
	     $post['thumbimage'] = $_POST['old_photo2'];
	    }*/
		//$post['fragnetid']=$_POST['hid_userid'];

			$editcms=$controller_class->editData('tbl_risk',$post,"riskid='".$_POST['hid_userid']."'");
...
<?php
	if (isset($_POST['viewdiv']) && $_POST['viewdiv'] != '') {
		...
		$qry = "SELECT * FROM tbl_risk where statuscheck !=2";
	...
	<div class="col-lg-12">
		<div class="widget-container fluid-height clearfix">
			<div class="heading"> <i style="cursor:default;" class="icon-table"></i> Risk List
			<?php
			 $selectmodule="SELECT * FROM tbl_modules_permission WHERE status='1' AND fk_admin_id= '".$_SESSION['TERRATROVE_ID']['ADMIN_ID']."' AND fk_permission_id = 10 ORDER BY id ASC";
			...
			?>
				...
				<div class="btn-group hidden-xs pull-right ajesh_margin_right_10">
					...
					<ul class="dropdown-menu">
						<li>
                            <a href="javascript:void(0)" onclick="statusactive('Active','active','Risk?')"><i class="icon-ok-sign"></i>Active</a>
						</li>
                        <li>
                            <a href="javascript:void(0)" onclick="statusinactive('Inactive','inactive','Risk?')"><i class="icon-ok-circle"></i>Inactive</a>
						</li>
						 <?php if($moduleresult['p_field4'] == 1){ ?>
						<li>
							<a href="javascript:void(0)" onclick="deleteselected('Delete','delete','Risk?')"><i class="icon-remove"></i>Delete</a>
						</li>
						<?php } ?>
					</ul>
				</div>
	...
	<script>
		$(document).ready(function() {
			$('#tbl_data_display').dataTable({
				...
				"sAjaxSource": site_url + 'controllers/ajax_controller/risk-ajax-controller.php?action=displaydata'
			});
		});
	</script>
...
<?php
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
		...
		$qry="UPDATE tbl_risk SET statuscheck=2 WHERE riskid='".$_POST['id']."'";
...
<?php
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
	 	$qry="SELECT * FROM tbl_risk WHERE riskid=".$_POST['id'];
			...
			<div class="widget-content padded">
				<form name="form_testadd" id="form_testadd" method="post" enctype="multipart/form-data" action="" class="form-horizontal">
					...
					<div class="form-group">
						...
						<div class="col-md-7">
						<input class="form-control" type="text" name="fragnetstitle" maxlength="240" id="fragnetstitle" value="<?php echo htmlentities(urldecode($data['risktitle'])); ?>" />
              ...
              <div class="col-md-4">
                <div class="fileupload fileupload-new" data-provides="fileupload">
                  <div class="fileupload-new img-thumbnail" style="width: 200px; height: 150px;">
                    <?php if($data['thumbimage'] == '' ){ ?>
                      <img src="http://www.placehold.it/200x150/EFEFEF/AAAAAA&text=no+image" >
                      <?php } else {
                        if($controller_class->isAws()){
                          $url = COFIG_SITE_URL_IMG.'risk_img/thumb/'.$data['thumbimage'];
                          }else{
								...
								<div class="fileupload fileupload-new" data-provides="fileupload">
									<div class="fileupload-new img-thumbnail" style="width: 200px; height: 150px;">
										<?php if($data['image'] == '' ){ ?>
											...
											<?php } else {
												if($controller_class->isAws()){
													$url = COFIG_SITE_URL_IMG.'risk_img/'.$data['image'];
													}else{
