<?php
  ...
  $upload_dir =$_SESSION['SITE_IMG_PATH']."forensic_img/";
  $upload_dir2 =$_SESSION['SITE_IMG_PATH']."forensic_img/thumb/";
  // $upload_dir3 =$_SESSION['SITE_IMG_PATH']."fragnets_img/";

	if(isset($_POST['view']) && $_POST['view'] != ''):
    $id = $_POST['id'];

    $qry="SELECT * FROM tbl_forensic left join tbl_forensic_image on tbl_forensic_image.fk_forensicid = tbl_forensic.forensicid where  tbl_forensic.forensicid=".$id;
    $result=$modelObj->fetchRow($qry);

?>
<div class="col-md-12">
    <div class="widget-content padded">
		<dl><b>Title : </b> <?php echo urldecode($result['forensictitle']); ?></dl>
    <?php
    if($result['image'] == '')
    {
    ...
    $imageviewthumb =  COFIG_SITE_URL_IMG."forensic_img/thumb/".$result['thumbimage'];
    }
     ?>
...
$imageview =  COFIG_SITE_URL_IMG."forensic_img/".$result['image'];
}
      ?>
...
<?php
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == "displaydata"){
		...
		if($iSortCol_0=='1'){
			$sortfield = 'forensictitle';
			}
		else{
				$sortfield = 'forensicid';
			}
		$TotalCountqry = $modelObj->numRows("select * FROM tbl_forensic");
    ...
    else  if(strtolower($sSearch) == 'inactive')
      {
        ...
        $sSearchactive = $sSearch;
				...
				$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_forensic  where statuscheck !=2 and (forensictitle LIKE '%".$sSearch."%' || statuscheck LIKE '%$sSearchactive%')");
			...
			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_forensic  where statuscheck !=2 and (forensictitle LIKE '%".$sSearch."%' || statuscheck LIKE '%$sSearchactive%') ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
			} else {
			$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_forensic left join tbl_forensic_image on tbl_forensic_image.fk_forensicid = tbl_forensic.forensicid where statuscheck !=2");

			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_forensic left join tbl_forensic_image on tbl_forensic_image.fk_forensicid = tbl_forensic.forensicid  where statuscheck !=2 ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
		}
		...
		foreach($list_activity_log as $result){
			...
			$query="SELECT  FROM tbl_forensic left join tbl_forensic_image on tbl_forensic_image.fk_forensicid = tbl_forensic.forensicid";
			$results=$modelObj->fetchRows($query);

			$cms_id = $result['forensicid'];
			$title = urldecode($result['forensictitle']);
			...
			$option .='<a class="table-actions" href="javascript:void(0)" onclick="edit('.$cms_id.',\'forensic\')" title="edit">
			<i class="icon-edit"></i>
			</a>
			</div>
			</td>';
			...
			<a class="table-actions" href="javascript:void(0)" onclick="edit('.$cms_id.',\'forensic\')" title="Edit">
		...
		$option='<td class="actions">
			...
			<a class="table-actions" href="javascript:void(0)" onclick="deleteuser('.$cms_id.', \'delete\', \'forensic\')" title="Delete">
...
<?php
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_forensic SET statuscheck=1 WHERE  forensicid=".($val);
...
<?php
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_forensic SET  statuscheck=0 WHERE  forensicid=".($val);
...
<?php
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val){
			$qry="UPDATE tbl_forensic SET  statuscheck=2 WHERE forensicid='".($val)."'";
    ...
    if(isset($_FILES["txt_addphoto"]["tmp_name"])){
        ...
        if(isset($_FILES["txt_addphoto2"]["tmp_name"])){
		  ...
		  $post['fk_language_id']=1;
			$post['forensictitle']=urlencode($_POST['forensictitle']);
		  $post['statuscheck']=1;
      $post1['image'] = $insertimage;
      $post1['thumbimage'] = $insertimagethumb;

	   	   $addfaq=$controller_class->addData('tbl_forensic',$post);
         $last_id 	= $GLOBALS['pdo']->lastInsertId();

         $post1['fk_forensicid'] = $last_id;

         $addfaq1=$controller_class->addData('tbl_forensic_image',$post1);
			echo '1';
    ...
    if($insertimagethumb == '')
    {
     $post1['thumbimage'] = $_POST['old_photo2'];
		...
		//$post['evmid']=$_POST['hid_userid'];
			...
			$editcms=$controller_class->editData('tbl_forensic',$post,"forensicid='".$_POST['hid_userid']."'");
      $editcms2=$controller_class->editData('tbl_forensic_image',$post1,"fk_forensicid='".$_POST['hid_userid']."'");
  		echo '1';
	}
?>
...
<?php
	if (isset($_POST['viewdiv']) && $_POST['viewdiv'] != '') {
		...
		$qry = "SELECT * FROM tbl_forensic left join tbl_forensic_image on tbl_forensic_image.fk_forensicid = tbl_forensic.forensicid where statuscheck !=2";
	...
	<div class="col-lg-12">
		<div class="widget-container fluid-height clearfix">
			<div class="heading"> <i style="cursor:default;" class="icon-table"></i> Forensic List
			<?php
			 $selectmodule="SELECT * FROM tbl_modules_permission WHERE status='1' AND fk_admin_id= '".$_SESSION['TERRATROVE_ID']['ADMIN_ID']."' AND fk_permission_id = 16 ORDER BY id ASC";
			...
			?>
				...
				<div class="btn-group hidden-xs pull-right ajesh_margin_right_10">
					...
					<ul class="dropdown-menu">
						<li>
                            <a href="javascript:void(0)" onclick="statusactive('Active','active','forensic?')"><i class="icon-ok-sign"></i>Active</a>
						</li>
                        <li>
                            <a href="javascript:void(0)" onclick="statusinactive('Inactive','inactive','forensic?')"><i class="icon-ok-circle"></i>Inactive</a>
						</li>
						 <?php if($moduleresult['p_field4'] == 1){ ?>
						<li>
							<!-- <a href="javascript:void(0)" onclick="deleteselected('Delete','delete','forensic?')"><i class="icon-remove"></i>Delete</a> -->
						</li>
						<?php } ?>
					</ul>
				</div>
	...
	<script>
		$(document).ready(function() {
			$('#tbl_data_display').dataTable({
				...
				"sAjaxSource": site_url + 'controllers/ajax_controller/forensic-ajax-controller.php?action=displaydata'
			});
		});
	</script>
...
<?php
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
		...
		$qry="UPDATE tbl_forensic SET statuscheck=2 WHERE 	forensicid='".$_POST['id']."'";
...
<?php
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
	 	$qry="SELECT * FROM tbl_forensic left join tbl_forensic_image on tbl_forensic_image.fk_forensicid = tbl_forensic.forensicid where tbl_forensic.forensicid=".$_POST['id'];
	...
	<div class="col-lg-12">
		<div class="widget-container fluid-height clearfix">
			<div class="heading"> <i style="cursor:default;" class="icon-reorder"></i>
				<?php if($_POST[ 'id']!=0){ ?> Update Forensic
					<?php } else { ?> Add Forensic
				<?php } ?>
			</div>
			<div class="widget-content padded">
				<form name="form_testadd" id="form_testadd" method="post" enctype="multipart/form-data" action="" class="form-horizontal">
					...
					<div class="form-group">
						...
						<div class="col-md-7">
						<input class="form-control" type="text" name="forensictitle"  id="forensictitle" value="<?php echo htmlentities(urldecode($data['forensictitle'])); ?>" />
						<span id="error_forensictitle" style="color:red" class="error_label"></span> </div>
					</div>
                    ...
                    <?php if($data['image'] == '' ){ ?>
                      ...
                      <?php } else {
                        if($controller_class->isAws()){
                          $url = COFIG_SITE_URL_IMG.'forensic_img/thumb/'.$data['thumbimage'];
                          }else{
								...
								<div class="fileupload fileupload-new" data-provides="fileupload">
									<div class="fileupload-new img-thumbnail" style="width: 200px; height: 150px;">
										<?php if($data['image'] == '' ){ ?>
											...
											<?php } else {
												if($controller_class->isAws()){
													$url = COFIG_SITE_URL_IMG.'forensic_img/'.$data['image'];
													}else{
