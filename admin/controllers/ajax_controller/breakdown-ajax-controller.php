<?php
  ...
  $upload_dir =$_SESSION['SITE_IMG_PATH']."breakdown_img/";
  $upload_dir2 =$_SESSION['SITE_IMG_PATH']."breakdown_img/thumb/";
  // $upload_dir3 =$_SESSION['SITE_IMG_PATH']."fragnets_img/";

	if(isset($_POST['view']) && $_POST['view'] != ''):
    $id = $_POST['id'];

   /* $qry="SELECT * FROM tbl_breakdown left join tbl_breakdown_image on tbl_breakdown_image.fk_breakdownid = tbl_breakdown.breakdownid where  tbl_breakdown.breakdownid=".$id;
    $result=$modelObj->fetchRow($qry);*/
    
    $qry="SELECT * FROM tbl_breakdown  where statuscheck!=2 AND breakdownid=".$id;
    $result=$modelObj->fetchRow($qry);

    $qry="SELECT * FROM tbl_breakdown_image  where fk_breakdownid=".$result['breakdownid'];
    $result1=$modelObj->fetchRow($qry);
   

?>
<div class="col-md-12">
    <div class="widget-content padded">
		<dl><b>Title : </b> <?php echo urldecode($result['breakdowntitle']); ?></dl>
    <?php
  if($result1['thumbimage'] == '')
  {
  $imageviewthumb = "http://www.placehold.it/200x150/EFEFEF/AAAAAA&text=no+image";
  }
  else {
  $imageviewthumb =  COFIG_SITE_URL_IMG."breakdown_img/thumb/".$result1['thumbimage'];
  }
     ?>
...
$imageview =  COFIG_SITE_URL_IMG."breakdown_img/".$result1['image'];
}
      ?>
...
<?php
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == "displaydata"){
		...
		if($iSortCol_0=='1'){
			$sortfield = 'breakdowntitle';
			}
		else{
				$sortfield = 'breakdownid';
			}
		$TotalCountqry = $modelObj->numRows("select * FROM tbl_breakdown");
    ...
    else  if(strtolower($sSearch) == 'inactive')
      {
        ...
        $sSearchactive = $sSearch;
				...
				$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_breakdown  where statuscheck !=2  and (breakdowntitle LIKE '%".$sSearch."%' || statuscheck LIKE '%$sSearchactive%')");
			...
			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_breakdown  where statuscheck !=2 and (breakdowntitle LIKE '%".$sSearch."%' || statuscheck LIKE '%$sSearchactive%') ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
			} else {
			$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_breakdown left join tbl_breakdown_image on tbl_breakdown_image.fk_breakdownid = tbl_breakdown.breakdownid where statuscheck !=2");

			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_breakdown left join tbl_breakdown_image on tbl_breakdown_image.fk_breakdownid = tbl_breakdown.breakdownid  where statuscheck !=2 ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
		}
		...
		foreach($list_activity_log as $result){
			...
			$query="SELECT  FROM tbl_breakdown left join tbl_breakdown_image on tbl_breakdown_image.fk_breakdownid = tbl_breakdown.breakdownid";
			$results=$modelObj->fetchRows($query);

			$cms_id = $result['breakdownid'];
			$title = urldecode($result['breakdowntitle']);
			...
			$option .='<a class="table-actions" href="javascript:void(0)" onclick="edit('.$cms_id.',\'breakdown\')" title="edit">
				<i class="icon-edit"></i>
			</a>';


			/*$option .='<a class="table-actions" href="javascript:void(0)" onclick="deleteuser('.$cms_id.', \'delete\', \'breakdown?\')" title="Delete">
			...
			<a class="table-actions" href="javascript:void(0)" onclick="edit('.$cms_id.',\'breakdown\')" title="Edit">
		...
		$option='<td class="actions">
			...
			<a class="table-actions" href="javascript:void(0)" onclick="deleteuser('.$cms_id.', \'delete\', \'breakdown\')" title="Delete">
			<i class="icon-trash"></i>
			</a>statuscheck
			</div>
			</td>';
...
<?php
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_breakdown SET statuscheck=1 WHERE  breakdownid=".($val);
...
<?php
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_breakdown SET  statuscheck=0 WHERE  breakdownid=".($val);
...
<?php
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val){
			$qry="UPDATE tbl_breakdown SET  statuscheck=2 WHERE breakdownid='".($val)."'";
    ...
    if(isset($_FILES["txt_addphoto"]["tmp_name"])){
        ...
        if(isset($_FILES["txt_addphoto2"]["tmp_name"])){
		  ...
		  $post['fk_language_id']=1;
			$post['breakdowntitle']=urlencode($_POST['breakdowntitle']);
		  $post['statuscheck']=1;
      $post1['image'] = $insertimage;
      $post1['thumbimage'] = $insertimagethumb;

	   	   $addfaq=$controller_class->addData('tbl_breakdown',$post);
         $last_id 	= $GLOBALS['pdo']->lastInsertId();

         $post1['fk_breakdownid'] = $last_id;

         $addfaq1=$controller_class->addData('tbl_breakdown_image',$post1);
			echo '1';
    ...
    if($insertimagethumb == '')
    {
     $post1['thumbimage'] = $_POST['old_photo2'];
		...
		//$post['evmid']=$_POST['hid_userid'];
			...
			$editcms=$controller_class->editData('tbl_breakdown',$post,"breakdownid='".$_POST['hid_userid']."'");
      $editcms2=$controller_class->editData('tbl_breakdown_image',$post1,"fk_breakdownid='".$_POST['hid_userid']."'");
  		echo '1';
	}
?>
...
<?php
	if (isset($_POST['viewdiv']) && $_POST['viewdiv'] != '') {
		...
		$qry = "SELECT * FROM tbl_breakdown left join tbl_breakdown_image on tbl_breakdown_image.fk_breakdownid = tbl_breakdown.breakdownid where statuscheck !=2";
			...
			?>
				...
				<div class="btn-group hidden-xs pull-right ajesh_margin_right_10">
					...
					<ul class="dropdown-menu">
						<li>
                            <a href="javascript:void(0)" onclick="statusactive('Active','active','breakdown?')"><i class="icon-ok-sign"></i>Active</a>
						</li>
                        <li>
                            <a href="javascript:void(0)" onclick="statusinactive('Inactive','inactive','breakdown?')"><i class="icon-ok-circle"></i>Inactive</a>
						</li>
						 <?php if($moduleresult['p_field4'] == 1){ ?>
						<!-- <li>
							<a href="javascript:void(0)" onclick="deleteselected('Delete','delete','breakdown?')"><i class="icon-remove"></i>Delete</a>
						</li> -->
						<?php } ?>
					</ul>
				</div>
	...
	<script>
		$(document).ready(function() {
			$('#tbl_data_display').dataTable({
				...
				"sAjaxSource": site_url + 'controllers/ajax_controller/breakdown-ajax-controller.php?action=displaydata'
			});
		});
	</script>
...
<?php
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
		...
		$qry="UPDATE tbl_breakdown SET statuscheck=2 WHERE 	breakdownid='".$_POST['id']."'";
...
<?php
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
	 	$qry="SELECT * FROM tbl_breakdown left join tbl_breakdown_image on tbl_breakdown_image.fk_breakdownid = tbl_breakdown.breakdownid where tbl_breakdown.breakdownid=".$_POST['id'];
	...
	<div class="col-lg-12">
		<div class="widget-container fluid-height clearfix">
			<div class="heading"> <i style="cursor:default;" class="icon-reorder"></i>
				<?php if($_POST[ 'id']!=0){ ?> Update Work Breakdown Structure
					<?php } else { ?> Add Work Breakdown Structure
				<?php } ?>
			</div>
			<div class="widget-content padded">
				<form name="form_testadd" id="form_testadd" method="post" enctype="multipart/form-data" action="" class="form-horizontal">
					...
					<div class="form-group">
						...
						<div class="col-md-7">
						<input class="form-control" type="text" name="breakdowntitle"  id="breakdowntitle" value="<?php echo htmlentities(urldecode($data['breakdowntitle'])); ?>" />
						<span id="error_breakdowntitle" style="color:red" class="error_label"></span> </div>
					</div>
          <!-- <div class="form-group">
                    ...
                    <?php if($data['image'] == '' ){ ?>
                      ...
                      <?php } else {
                        if($controller_class->isAws()){
                          $url = COFIG_SITE_URL_IMG.'breakdown_img/thumb/'.$data['thumbimage'];
                          }else{
								...
								<div class="fileupload fileupload-new" data-provides="fileupload">
									<div class="fileupload-new img-thumbnail" style="width: 200px; height: 150px;">
										<?php if($data['image'] == '' ){ ?>
											...
											<?php } else {
												if($controller_class->isAws()){
													$url = COFIG_SITE_URL_IMG.'breakdown_img/'.$data['image'];
													}else{
