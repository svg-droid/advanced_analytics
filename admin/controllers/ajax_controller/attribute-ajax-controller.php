<?php 
	...
	include('../../models/attribute-model.php');
    include('../../includes/thumb_new.php');
    include('../../includes/resize-class.php');
    include('../common-controller.php');
	include('../attribute-controller.php');
    include('../../models/ajax-model.php');
    $modelObj = new AjaxModel();
	$controller_class = new CommonController();
	$upload_dir1 =$_SESSION['SITE_IMG_PATH']."attribute/";
    $upload_dir =$_SESSION['SITE_IMG_PATH']."attribute/";
    $upload_dirthumb =$_SESSION['SITE_IMG_PATH']."attribute/thumb/";
	

if(isset($_POST['view']) && $_POST['view'] != ''):
    $id = $_POST['id'];
    $qry="SELECT * FROM tbl_attribute where status!=2 and id='".$id."'";
    $result=$modelObj->fetchRow($qry);
?>
<div class="col-md-12">
    <div class="widget-content padded">
		<dl><b> Attribute Category Name : </b> <?php echo $result['attribute_name']; ?></dl> 
...
if(isset($_REQUEST['action']) && $_REQUEST['action'] == "displaydata"){
  ...
  $TotalCountqry = $modelObj->numRows("select * FROM tbl_attribute WHERE status!=2");
  
  
 
	if($sSearch!=''){
		$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_attribute WHERE status!=2 AND attribute_name like '%".$sSearch."%'");
		
		$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_attribute WHERE status!=2 AND attribute_name LIKE '%".$sSearch."%' ORDER BY createdatetime ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");	
	} else {
		$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_attribute WHERE status!=2");
		
		$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_attribute WHERE status!=2 ORDER BY createdatetime ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");	
	}
  ...
  foreach($list_activity_log as $result){
   $cms_id = $result['id'];
   $cms_page_name = $result['attribute_name'];
   $cms_status = $result['status'];
		...
		$option='<td class="actions">
					<div class="action-buttons">
						...
						<a class="table-actions" href="javascript:void(0)" onclick="edit('.$cms_id.',\'attribute\')" title="Edit">
						 <i class="icon-pencil"></i>
						</a>
							
					<a class="table-actions" href="javascript:void(0)" onclick="deleteuser('.$cms_id.', \'delete\', \'attribute?\')" title="Delete">
							   <i class="icon-trash"></i>
					</a>
					</div>
				</td>';
   
   $Arr['aaData'][] = array($checkbox,$name,$status,$option);
  } 
...
<?php
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_attribute SET status=1 WHERE id=".($val);
...
<?php
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_attribute SET status=0 WHERE id=".($val);
			$result =$modelObj->runQuery($qry);
		}
	}
?>
<?php
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val){
			$qry="UPDATE tbl_attribute SET status=2 WHERE id='".$val."'";
			$result=$modelObj->runQuery($qry);
		}
	}
?>
...
<?php
    ...
    $checkexists="SELECT id FROM tbl_attribute where status!=2 and attribute_name='".($_POST['attribute_name'])."'";        
        ...
        if($flag==1)
        {
            ...
            $post['attribute_name']=strip_tags($_POST['attribute_name']);
            $post['status']='1';
            ...
            $addfaq=$controller_class->addData('tbl_attribute',$post);
...
<?php
    if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){  
        $checkexists="SELECT id FROM tbl_attribute WHERE attribute_name='".(trim($_POST['attribute_name']))."' and status!=2 and id!='".$_POST['hid_userid']."'";
        $result_checkexists=$modelObj->numRows($checkexists);
        if($result_checkexists>0){
            $flag=0;
            echo "0";
            ...
            $editcms=$controller_class->editData('tbl_attribute',$post,"id='".$post['id']."'");
            echo '1';
        }
    }   
?>
...
<?php
    ...
    $qry = "SELECT * FROM tbl_attribute where status!=2 order by createdatetime desc";
...
<div class="col-lg-12">
    <div class="widget-container fluid-height clearfix">
        ...
        <?php
            $selectmodule="SELECT * FROM tbl_modules_permission WHERE status='1' AND fk_admin_id= '".$_SESSION['TERRATROVE_ID']['ADMIN_ID']."' AND fk_permission_id = 4 ORDER BY id ASC";
            ...
            <div class="btn-group hidden-xs pull-right ajesh_margin_right_10">
                ...
                <ul class="dropdown-menu">
                    <li>
                            <a href="javascript:void(0)" onclick="statusactive('Active','active','attribute?')"><i class="icon-ok-sign"></i>Active</a>
                        </li>
                        <li>
                            <a href="javascript:void(0)" onclick="statusinactive('Inactive','inactive','attribute?')"><i class="icon-ok-circle"></i>Inactive</a>
                        </li>
                </ul>
            </div>
            <?php endif;?> </div>
            ...
            <table class="table table-bordered table-striped" id="tbl_data_display">
                <thead>
                    <th class="check-header hidden-xs">
                        ...
                        <th class="hidden-xs" style="color: #007aff;">Attribute Name</th>
...
<script>
    $(document).ready(function() {
        $('#tbl_data_display').dataTable({
            ...
            "sAjaxSource": site_url + 'controllers/ajax_controller/attribute-ajax-controller.php?action=displaydata'
        });
    });
</script>
...
<div class="col-lg-12">
    <div class="widget-container fluid-height clearfix">
        <div class="heading"> <i style="cursor:default;" class="icon-reorder"></i>
            <?php if($_POST[ 'id']!=0){ ?> Update Attribute
            <?php } else { ?> Add Attribute
            <?php } ?>
        </div>
        <div class="widget-content padded">
            <form name="form_cmsadd" id="form_cmsadd" method="post" enctype="multipart/form-data" action="" class="form-horizontal">
                <div class="form-group">
                    ...
                    <div class="col-md-7">
                        <input class="form-control" type="text" name="attribute_name" maxlength="40" id="attribute_name" value="<?php echo stripslashes($data['attribute_name']); ?>" /> <span id="error_attribute_name" style="color:red" class="error_label"></span> </div>
                </div>
