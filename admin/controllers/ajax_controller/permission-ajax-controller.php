<?php 
	...
	include('../../models/permission-model.php');
    include('../../includes/thumb_new.php');
    include('../../includes/resize-class.php');
    include('../common-controller.php');
	include('../permission-controller.php');
    include('../../models/ajax-model.php');
    $modelObj = new AjaxModel();
	$controller_class = new CommonController();
	$upload_dir1 =$_SESSION['SITE_IMG_PATH']."permission/";
    $upload_dir =$_SESSION['SITE_IMG_PATH']."permission/";
    $upload_dirthumb =$_SESSION['SITE_IMG_PATH']."permission/thumb/";
	
	if(isset($_POST['view']) && $_POST['view'] != ''):
    $id = $_POST['id'];
    $qry="SELECT * FROM tbl_permission where id=".$id;
    $result=$modelObj->fetchRow($qry);
    
?>
<div class="col-md-12">
    <div class="widget-content padded">
		<dl><b>Module Name : </b> <?php echo  $result['modulename']; ?></dl> 
...
<?php
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == "displaydata"){
		...
		$TotalCountqry = $modelObj->numRows("select * FROM tbl_permission where status!=2");
		
		
		
		if($sSearch!=''){
			$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_permission where status!=2 AND modulename LIKE '%".$sSearch."%'");
			
			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_permission where status!=2 AND modulename LIKE '%".$sSearch."%' LIMIT ".$iDisplayStart.",".$iDisplayLength."");	
			} else {
			$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_permission where status!=2 ");
			
			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_permission where status!=2  LIMIT ".$iDisplayStart.",".$iDisplayLength."");	
		}
		...
		foreach($list_activity_log as $result){
			...
			$cms_page_name = $result['modulename'];
		...
		if($status==1)
			...
			<a class="table-actions" href="javascript:void(0)" onclick="edit('.$cms_id.',\'permission\')" title="Edit">
			<i class="icon-pencil"></i>
			</a>
			
			<a class="table-actions" href="javascript:void(0)" onclick="deleteuser('.$cms_id.', \'delete\', \'permission\')" title="Delete">
...
<?php
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_permission SET status=1 WHERE id=".($val);
...
<?php
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_permission SET status=0 WHERE id=".($val);
			$result =$modelObj->runQuery($qry);
...
<?php
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val){
			$qry="UPDATE tbl_permission SET status=2 WHERE id='".$val."'";
			$result=$modelObj->runQuery($qry);
...
<?php
	...
	$checkexists="SELECT id FROM tbl_permission where status!=2 and modulename='".($_POST['modulename'])."'";		
		...
		if($flag==1)
		{
			...
			$post['modulename']=strip_tags($_POST['modulename']);
			$post['status']='1';
			...
			$addfaq=$controller_class->addData('tbl_permission',$post);
...
<?php
	if(isset($_POST['hid_update']) && $_POST['hid_update'] != ''){	
			$checkexists="SELECT id FROM tbl_permission WHERE modulename='".(trim($_POST['modulename']))."' and status!=2 and id!='".$_POST['hid_userid']."'";
		$result_checkexists=$modelObj->numRows($checkexists);
		if($result_checkexists>0){
			$flag=0;
			echo "0";
			...
			$editcms=$controller_class->editData('tbl_permission',$post,"id='".$post['id']."'");
			echo '1';
...
<?php
	if (isset($_POST['viewdiv']) && $_POST['viewdiv'] != '') {
		...
		$qry = "SELECT * FROM tbl_permission ";
	...
	<div class="col-lg-12">
		<div class="widget-container fluid-height clearfix">
			<div class="heading"> <i style="cursor:default;" class="icon-table"></i> Permission List
				...
				<div class="btn-group hidden-xs pull-right ajesh_margin_right_10">
					...
					<ul class="dropdown-menu">
						<li>
                            <a href="javascript:void(0)" onclick="statusactive('Active','active','permission')"><i class="icon-ok-sign"></i>Active</a>
						</li>
                        <li>
                            <a href="javascript:void(0)" onclick="statusinactive('Inactive','inactive','permission')"><i class="icon-ok-circle"></i>Inactive</a>
						</li>
						<li>
							<a href="javascript:void(0)" onclick="deleteselected('Delete','delete','permission')"><i class="icon-remove"></i>Delete</a>
						</li>
					</ul>
				</div>
			<?php endif;?> </div>
	...
	<script>
		$(document).ready(function() {
			$('#tbl_data_display').dataTable({
				...
				"sAjaxSource": site_url + 'controllers/ajax_controller/permission-ajax-controller.php?action=displaydata'
			});
		});
	</script>
...
<?php
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
		...
		$qry="UPDATE tbl_permission SET status=2 WHERE id='".$_POST['id']."'";
		$result=$modelObj->runQuery($qry);
...
<?php
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
		$qry="SELECT * FROM tbl_permission WHERE id=".$_POST['id'];
	...
	<div class="col-lg-12">
		<div class="widget-container fluid-height clearfix">
			<div class="heading"> <i style="cursor:default;" class="icon-reorder"></i>
				<?php if($_POST[ 'id']!=0){ ?> Update Permission
					<?php } else { ?> Add Permission
				<?php } ?>
			</div>
			<div class="widget-content padded">
				<form name="form_cmsadd" id="form_cmsadd" method="post" enctype="multipart/form-data" action="" class="form-horizontal">
					...
					<div class="form-group">
						<label class="control-label col-md-2">Module Name <font class="required_mark" color="red">*</font>
						</label>
						<div class="col-md-7">
						<input class="form-control" type="text" name="modulename" maxlength="40" id="modulename" value="<?php echo stripslashes($data['modulename']); ?>" /> <span id="error_modulename" style="color:red" class="error_label"></span> </div>
