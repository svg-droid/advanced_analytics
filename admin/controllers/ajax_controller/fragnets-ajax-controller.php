<?php
  ...
  $upload_dir =$_SESSION['SITE_IMG_PATH']."fragnets_img/";
  $upload_dir2 =$_SESSION['SITE_IMG_PATH']."fragnets_img/thumb/";
  ...
  // $upload_dir3 =$_SESSION['SITE_IMG_PATH']."fragnets_img/";
   ...
   $qry="SELECT * FROM tbl_fragnets left join tbl_fragnets_image on tbl_fragnets_image.fk_fragnetid = tbl_fragnets.fragnetid where  tbl_fragnets.fragnetid=".$id;
    $result=$modelObj->fetchRow($qry);

?>
<div class="col-md-12">
    <div class="widget-content padded">
		<dl><b>Title : </b> <?php echo urldecode($result['fragnettitle']); ?></dl>
    <?php
if($result['image'] == '')
{
...
$imageviewthumb =  COFIG_SITE_URL_IMG."fragnets_img/thumb/".$result['thumbimage'];
}
     ?>
...
$imageview =  COFIG_SITE_URL_IMG."fragnets_img/".$result['image'];
}
      ?>
     <dl><b>Main Image : </b><br/> <img src="<?php echo  $imageview; ?>"  height="100" width="100"/></dl>
	</div>
</div>
<?php endif; ?>
<?php
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == "displaydata"){
		...
		if($iSortCol_0=='1'){
  			$sortfield = 'fragnettitle';
			}
		else{
				$sortfield = 'fragnetid';
			}
		$TotalCountqry = $modelObj->numRows("select * FROM tbl_fragnets");
    ...
    else  if(strtolower($sSearch) == 'inactive')
      {
        ...
        $sSearchactive = $sSearch;
		  ...
		  $Filteredqry = $modelObj->numRows("SELECT * FROM tbl_fragnets  where statuscheck !=2 and (fragnettitle LIKE '%".$sSearch."%' || statuscheck LIKE '%$sSearchactive%')");
			...
			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_fragnets  where statuscheck !=2 and (fragnettitle LIKE '%".$sSearch."%' || statuscheck LIKE '%$sSearchactive%') ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
			} else {
			$Filteredqry = $modelObj->numRows("SELECT * FROM tbl_fragnets left join tbl_fragnets_image on tbl_fragnets_image.fk_fragnetid = tbl_fragnets.fragnetid where statuscheck !=2");

			$list_activity_log = $modelObj->fetchRows("SELECT * FROM tbl_fragnets left join tbl_fragnets_image on tbl_fragnets_image.fk_fragnetid = tbl_fragnets.fragnetid  where statuscheck !=2 ORDER BY ".$sortfield." ".$SortBy." LIMIT ".$iDisplayStart.",".$iDisplayLength."");
		}
		...
		foreach($list_activity_log as $result){
			...
			$query="SELECT  FROM tbl_fragnets left join tbl_fragnets_image on tbl_fragnets_image.fk_fragnetid = tbl_fragnets.fragnetid";
			$results=$modelObj->fetchRows($query);

			$cms_id = $result['fragnetid'];
			$title = urldecode($result['fragnettitle']);
		  $image = $result['image'];
			...
			$cms='<td>'.$title.'</td>';
		//	$cmsemail='<td><img height="100" width="100" src='.COFIG_SITE_URL_IMG.'fragnets_img/'.$image.'></td>';
			...
			$status_new ='<td class="hidden-xs"><span class="label label-danger">Inactive</span></td>';
			 ...
			 $selectmodule="SELECT * FROM tbl_modules_permission WHERE status='1' AND fk_admin_id= '".$_SESSION['TERRATROVE_ID']['ADMIN_ID']."' AND fk_permission_id = 10 ORDER BY id ASC";
			...
			$option .='<a class="table-actions" href="javascript:void(0)" onclick="edit('.$cms_id.',\'fragnets\')" title="edit">
			<i class="icon-edit"></i>
			</a>';


			/*$option .='<a class="table-actions" href="javascript:void(0)" onclick="deleteuser('.$cms_id.', \'delete\', \'fragnets?\')" title="Delete">
...
<?php
	if(isset($_POST['statusactive']) && $_POST['statusactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_fragnets SET statuscheck=1 WHERE  	fragnetid=".($val);
...
<?php
	if(isset($_POST['statusinactive']) && $_POST['statusinactive'] != '')
	{
		...
		foreach($id as $k => $val)
		{
			$qry="UPDATE tbl_fragnets SET  statuscheck=0 WHERE  fragnetid=".($val);
...
<?php
	if(isset($_POST['deleselected']) && $_POST['deleselected'] != ''){
		...
		foreach($id as $k => $val){
			$qry="UPDATE tbl_fragnets SET  statuscheck=2 WHERE fragnetid='".($val)."'";
    ...
    if(isset($_FILES["txt_addphoto"]["tmp_name"])){
        ...
        if(isset($_FILES["txt_addphoto2"]["tmp_name"])){
		  ...
		  $post['fk_language_id']=1;
			$post['fragnettitle']=urlencode($_POST['fragnetstitle']);
		  $post['statuscheck']=1;
      $post1['image'] = $insertimage;
      $post1['thumbimage'] = $insertimagethumb;

	   	   $addfaq=$controller_class->addData('tbl_fragnets',$post);
         $last_id 	= $GLOBALS['pdo']->lastInsertId();

         $post1['fk_fragnetid'] = $last_id;

         $addfaq1=$controller_class->addData('tbl_fragnets_image',$post1);
			echo '1';
...
<?php
    ...
    if($insertimagethumb == '')
    {
     $post1['thumbimage'] = $_POST['old_photo2'];
		...
		//$post['fragnetid']=$_POST['hid_userid'];

			$editcms=$controller_class->editData('tbl_fragnets',$post,"fragnetid='".$_POST['hid_userid']."'");
      $editcms2=$controller_class->editData('tbl_fragnets_image',$post1,"fk_fragnetid='".$_POST['hid_userid']."'");
  		echo '1';
...
<?php
	if (isset($_POST['viewdiv']) && $_POST['viewdiv'] != '') {
		...
		$qry = "SELECT * FROM tbl_fragnets left join tbl_fragnets_image on tbl_fragnets_image.fk_fragnetid = tbl_fragnets.fragnetid where statuscheck !=2";
			...
			?>
				...
				<div class="btn-group hidden-xs pull-right ajesh_margin_right_10">
					...
					<ul class="dropdown-menu">
						<li>
                            <a href="javascript:void(0)" onclick="statusactive('Active','active','fragnets?')"><i class="icon-ok-sign"></i>Active</a>
						</li>
                        <li>
                            <a href="javascript:void(0)" onclick="statusinactive('Inactive','inactive','fragnets?')"><i class="icon-ok-circle"></i>Inactive</a>
						</li>
						 <?php if($moduleresult['p_field4'] == 1){ ?>
						<!-- <li>
							<a href="javascript:void(0)" onclick="deleteselected('Delete','delete','fragnets?')"><i class="icon-remove"></i>Delete</a>
						</li> -->
						<?php } ?>
					</ul>
				</div>
	...
	<script>
		$(document).ready(function() {
			$('#tbl_data_display').dataTable({
				...
				"sAjaxSource": site_url + 'controllers/ajax_controller/fragnets-ajax-controller.php?action=displaydata'
			});
		});
	</script>
...
<?php
	if(isset($_POST['delete']) && $_POST['delete'] != ''){
		...
		$qry="UPDATE tbl_fragnets SET statuscheck=2 WHERE 	fragnetid='".$_POST['id']."'";
...
<?php
	if(isset($_POST['edit']) && $_POST['edit'] != ''){
	 	$qry="SELECT * FROM tbl_fragnets left join tbl_fragnets_image on tbl_fragnets_image.fk_fragnetid = tbl_fragnets.fragnetid where tbl_fragnets.fragnetid=".$_POST['id'];
	...
	<div class="col-lg-12">
		<div class="widget-container fluid-height clearfix">
			<div class="heading"> <i style="cursor:default;" class="icon-reorder"></i>
				<?php if($_POST[ 'id']!=0){ ?> Update Fragnets
					<?php } else { ?> Add Fragnets
				<?php } ?>
			</div>
			<div class="widget-content padded">
				<form name="form_testadd" id="form_testadd" method="post" enctype="multipart/form-data" action="" class="form-horizontal">
					...
					<div class="form-group">
						...
						<div class="col-md-7">
						<input class="form-control" type="text" name="fragnetstitle" maxlength="240" id="fragnetstitle" value="<?php echo htmlentities(urldecode($data['fragnettitle'])); ?>" />
						<span id="error_fragnetstitle" style="color:red" class="error_label"></span> </div>
					</div>
         ...
         <!--  <div class="form-group">
              ...
              <div class="col-md-4">
                    ...
                    <?php if($data['image'] == '' ){ ?>
                      ...
                      <?php } else {
                        if($controller_class->isAws()){
                          $url = COFIG_SITE_URL_IMG.'fragnets_img/thumb/'.$data['thumbimage'];
                          }else{
                ...
                <!-- <br/>	<span id="error_txt_addphoto2" style="color:red" class="error_label"></span>
					...
					<div class="form-group">
							<label class="control-label col-md-2">Main Image<font class="required_mark" color="red">*</font></label>
							<div class="col-md-4">
								<div class="fileupload fileupload-new" data-provides="fileupload">
									<div class="fileupload-new img-thumbnail" style="width: 200px; height: 150px;">
										<?php if($data['image'] == '' ){ ?>
											...
											<?php } else {
												if($controller_class->isAws()){
													$url = COFIG_SITE_URL_IMG.'fragnets_img/'.$data['image'];
													}else{
