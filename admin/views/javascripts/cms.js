function adddata(){
	var flag = 0; 
	   ...
	   if(requiredValidate('AddLang',AddLangMsg)){
            flag++;
        } 
	   if(requiredValidate('cms_page_name',cmsMsg)){
			flag++;
		}     
		if(requiredValidateckdior('cms_page_content',cmspageMsg)){
			flag++;
		}   
		/*if(requiredValidate('meta_title',metatitleMsg)){
			flag++;
		} 
		if(requiredValidate('meta_description',metadescriptionMsg)){
			flag++;
		}  
		if(requiredValidate('meta_keyword',metakeywordMsg)){
			flag++;
		} */   
		if(flag>0){
			return false;
