function adddata(){
	var flag = 0;  
	if(requiredValidate('AddAdmin',AddAdminMsg)){
            flag++;
        } 
        if(requiredValidate('txt_add',AddAddMsg)){
            flag++;
        } 
          if(requiredValidate('txt_view',AddViewMsg)){
            flag++;
        } 
          if(requiredValidate('txt_edit',AddEditMsg)){
            flag++;
        } 
          if(requiredValidate('txt_delete',AddDeleteMsg)){
            flag++;
        } 
		...
		if(flag>0){
			...
			var options = {
				...
				url:       site_url+'controllers/ajax_controller/permissionmodule-ajax-controller.php', 
				type: "POST"
			};
