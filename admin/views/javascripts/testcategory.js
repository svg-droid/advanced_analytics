function adddata(){
	var flag = 0;
	if(requiredValidate('categoryname',categorynameMsg)){
            flag++;
}
	...
	if(flag>0){
			...
			var options = {
				...
				url:       site_url+'controllers/ajax_controller/testcategory-ajax-controller.php',
				type: "POST"
			};
...
function showResponse(data, statusText)  {
	if(statusText == 'success'){
		if(data == 0){
			...
			$("#message-red").show().fadeOut(7000);
			$("#message-green").hide();
			document.getElementById('err').innerHTML='Category name already exist,Please try another name.';
		}
	}
