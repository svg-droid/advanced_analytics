function adddata(){
	...
	if(requiredValidate('name',marketingMsg)){
			flag++;
		}
		if(requiredValidate('addcountry',CountrySelectMsg)){
			flag++;
		} 
		if(requiredValidate('link',marketingLinkMsg)){
			flag++;
		}     
	if(regexValidate('image',marketingImg,photoValidMsg,'img')){
		   flag++;
		 }
...
function updatedata(){
   ...
   if(img!='' && regexValidate('image',marketingImg,photoValidMsg,'img')){
		   flag++;
		 }
		if(flag>0){
			return false;
