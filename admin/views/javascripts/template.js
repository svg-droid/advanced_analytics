function adddata(){
	var flag = 0;       
	 ...
	 if(requiredValidate('templatename',templatenameMsg)){
			flag++;
		}
	if(requiredValidateckdior('description',tempdescriptionMsg)){
			flag++;
		}  
		if(flag>0){
			return false;
			...
			var options = {
				...
				url:       site_url+'controllers/ajax_controller/template-ajax-controller.php', 
				type: "POST"
			};
	...
	var flag = 0;
	 if(requiredValidate('AddLang',AddLangMsg)){
       ...
       if(requiredValidate('templatename',templatenameMsg)){
		...
		if(requiredValidateckdior('description',descriptionMsg)){
			flag++;
		}  
