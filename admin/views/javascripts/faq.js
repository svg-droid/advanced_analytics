function adddata(){
	var flag = 0; 
	   ...
	   if(requiredValidate('title',faqMsg)){
			flag++;
		}     
		
		if(requiredValidate('meta_description',descriptionfaqMsg)){
			flag++;
		}  
		...
		if(flag>0){
			...
			var options = {
				...
				url:       site_url+'controllers/ajax_controller/faq-ajax-controller.php', 
				type: "POST"
			};
	...
	var flag = 0;
	 if(requiredValidate('AddLang',AddLangMsg)){
	   ...
	   if(requiredValidate('title',faqMsg)){
		...
		if(requiredValidate('meta_description',descriptionMsg)){
			flag++;
		}  
