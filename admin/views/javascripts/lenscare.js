function adddata(){
	...
	//alert(video);
	//var url = $('').val();
		if(requiredValidate('titles',titlesmsg)){
       	 flag++;
  		 }  
  		 if(requiredValidate('type_name',typedMsg)){
       	 flag++;

  		 }       
  		 if(requiredValidate('short_description',shortDescMsg)){
       	 flag++;
  		 }
		if(flag>0){
			return false;
			...
			var options = {
				...
				url:       site_url+'controllers/ajax_controller/lenscare-ajax-controller.php', 
				type: "POST"
			};
