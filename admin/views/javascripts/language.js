function adddata(){
	...
	if(requiredValidate('languagename',languagenameMsg)){
			flag++;
		} 
		if(requiredValidate('shortcode',shortcodeMsg)){
			flag++;
		} 
		    
	if(regexValidate('image',LangImage,langValidMsg,'img')){
		   flag++;
		 }
		...
		if(flag>0){
			...
			var options = {
				...
				url:       site_url+'controllers/ajax_controller/language-ajax-controller.php', 
				type: "POST"
			};
...
function updatedata(){
	...
	var img=$("#image").val();     
	  ...
	  if(img!='' && regexValidate('image',LangImage,langValidMsg,'img')){
		   flag++;
		 }
		if(flag>0){
			return false;
