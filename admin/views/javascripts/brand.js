function adddata(){
	...
	var flag = 0;
	if(requiredValidate('brand_type',brandtypemsg)){
			flag++;
		}
	if(requiredValidate('brand1',brand1Msg)){
			flag++;
		}
	 
    if(requiredValidate('txtcountry',CountrySelectMsg)){
		flag++;
	} 
	if(regexValidate('txt_addphoto','Please select brand logo','Please Valid Format','img')){
			flag++;
		}    
	if(regexValidate('txt_addphoto1','Please select brand banner image','Please Valid Format','img')){
			flag++;
		}   
...
function updatedata(){
	...
	/*if(regexValidate('image',BrandImg,photoValidMsg,'img')){
		   flag++;
	 }*/
		if(flag>0){
			return false;
