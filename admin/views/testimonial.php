<?php 
    session_start();
    $selectmodule="SELECT * FROM tbl_modules_permission WHERE status='1' AND fk_admin_id= '".$_SESSION['TERRATROVE_ID']['ADMIN_ID']."' AND fk_permission_id = 14 ORDER BY id ASC";
    $moduleresult=$controller_class->modelObj->fetchRow($selectmodule); 
    if($moduleresult['p_field1'] == 1 || $moduleresult['p_field2'] == 1 || $moduleresult['p_field3'] == 1 || $moduleresult['p_field4'] == 1){
    
?>
<script type="text/javascript" src="<?php echo $LOCATION['SITE_ADMIN'];?>views/javascripts/testimonial.js?<?php echo time(); ?>"></script>
...
<div class="row" id="<?php echo $_GET['pid']; ?>">
    <div class="col-lg-12">
        <div class="widget-container fluid-height clearfix">
            <div class="heading">
                <i style="cursor:default;" class="icon-table"></i> Testimonial List
                <?php
                  if($moduleresult['p_field2'] == 1){
                     ?>
                <a class="btn btn-sm btn-primary pull-right ajesh_margin_right_0" href="javascript:void(0)" onclick="showadd1('testimonial')"><i class="icon-plus"></i>Add New</a>
                 <?php } ?>
